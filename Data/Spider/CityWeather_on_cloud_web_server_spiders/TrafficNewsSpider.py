...
class TrafficNewsSpider(object):
    def __init__(self):
    ...
    # 9. 插入交通新闻内容
    def insert_traffic_news(self,all_list):
        # 插入'\链接\标题\分类\时间\来源\关键词\正文\作者\图片链接'
        ins='insert into traffic (newsurl,title,categoryid,time,source,tag,context,author,imgurl,part) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
        try:
    ...
    # 8. 获取交通新闻链接
    def get_traffic_link(self):
        response = requests.get(url=self.traffic_url,headers=self.headers)
        ...
        for nlink in nlink_list:
            ...
            if not self.is_news_exist(finger):
                # 判断链接是否为新闻,不是则跳过
                all_list = self.parse_traffic_news_page(nlink)
                if all_list:
                    self.insert_traffic_news(all_list)
    ...
    # 解析交通新闻页面
    def parse_traffic_news_page(self, nlink):
    ...
    def run(self):
        #爬取新闻
        self.get_traffic_link()

if __name__ == '__main__':
    for i in range(3):
        try:
            spider=TrafficNewsSpider()
            spider.run()
            with open('/home/ubuntu/logs/HexuWeather/spider/python_logs/TrafficNews_insert.log', 'a') as f:
