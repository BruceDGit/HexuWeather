...
class TravelNewsSpider(object):
    def __init__(self):
        self.url = 'https://news.sina.com.cn/'
        self.travel_url = 'http://travel.enorth.com.cn/'
        self.weather_url='http://news.enorth.com.cn/tj/fwkx/tianqixinxi/'
        self.traffic_url='http://news.enorth.com.cn/tj/fwkx/jiaotongxinxi/'
    ...
    # 7. 插入旅游新闻内容
    def insert_travel_news(self,all_list):
        # 插入'\链接\标题\分类\时间\来源\关键词\正文\作者\图片链接'
        ins='insert into travelnews (newsurl,title,categoryid,time,source,tag,content,author,imgurl,part) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
        try:
    ...
    # 8. 获取旅游新闻链接
    def get_travle_link(self):
        one_html = self.get_html(self.travel_url)
        xpath_bds = '//div[@class="menu1"]//a/@href'
        ...
        for nlink in nlink_list:
            ...
            if not self.is_news_exist(finger):
                # 判断链接是否为新闻,不是则跳过
                all_list = self.parse_travel_news_page(nlink)
                if all_list:
                    self.insert_travel_news(all_list)
    ...
    # 解析旅游新闻页面
    def parse_travel_news_page(self,nlink):
        response = requests.get(nlink, headers=self.headers)
        response.encoding = "gbk"
        html = response.text
        # 新闻字段解析式
        title_bds = '//*[@id="title"]/div[1]/h2/b/text()'
    ...
    def run(self):
        #爬取新闻
        self.get_travle_link()


if __name__ == '__main__':
    for i in range(3):
        try:
            spider=TravelNewsSpider()
            spider.run()
            with open('/home/ubuntu/logs/HexuWeather/spider/python_logs/TravelNews_insert.log', 'a') as f:
